...
def cornersHeuristic(state, problem):
    ...
    dist = 0
    xy1 = state[0]
    notChecked = list(state[1][:])
    
    while not len(notChecked)==0:
        dict = {}
        for x in notChecked:
            xy2 = x
            currDist = ( (xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2 ) ** 0.5 #euclidian
            dict[x] = currDist
            xy1 = x
        minDist = dict[min(dict)]
        dist = dist + minDist
        notChecked.remove (min(dict))
...
def foodHeuristic(state, problem):
    ...
    for i in food:
        currDist = mazeDistance (position, i, startState)
        if currDist  > dist:
            dist = currDist 
